...
vps_maker() {
    ...
    echo -e "${LIGHT_GREEN}              VPS MAKER SETUP${RESET}"
    echo

    echo -e "${YELLOW}[1/3] Updating package lists...${RESET}"
    echo
    ...
    echo
    echo -e "${YELLOW}[2/3] Installing VPS dependencies...${RESET}"
    echo
    ...
    echo
    echo -e "${YELLOW}[3/3] Starting VPS Maker...${RESET}"
    echo

    VPS_SCRIPT_URL="https://raw.githubusercontent.com/NeelAhuja1234/kvm-vps/main/nokvm.sh"

    bash <(curl -fsSL "$VPS_SCRIPT_URL")
    ...
    if [ "$EXIT_CODE" -ne 0 ]; then
        echo -e "${RED}✗ VPS Maker exited with an error.${RESET}"
    else
        echo -e "${GREEN}✓ VPS Maker closed successfully.${RESET}"
    fi

    echo
    echo -e "${YELLOW}Returning to NEELCRAFT...${RESET}"
