...
download_image() {
    ...
    echo
    echo -e "${CYAN}Downloading Ubuntu image...${RESET}"
...
get_vm_details() {
    ...
    echo -e "${YELLOW}NO-KVM VPS CONFIGURATION${RESET}"
    echo

    echo "This mode uses software emulation."
    echo "Performance may be significantly slower."

    echo

    read -rp "VM Name: " VM_NAME
    read -rp "RAM (MB) [1024]: " VM_RAM
    read -rp "CPU Cores [1]: " VM_CPU
    read -rp "Disk (GB) [15]: " VM_DISK
    ...
    if ! validate_number "$VM_RAM" ||
       ...
       ! validate_number "$SSH_PORT"; then
        ...
        echo -e "${RED}Numeric values are required.${RESET}"
...
check_port() {
    ...
    if ss -lnt 2>/dev/null | \
        ...
        echo -e "${RED}Port already in use.${RESET}"
...
create_disk() {
    ...
    if [ -f "$VM_DISK_PATH" ]; then
        ...
        echo -e "${RED}VM disk already exists.${RESET}"
    ...
    echo
    echo -e "${GREEN}Creating disk...${RESET}"
...
start_vm() {
    ...
    echo
    echo -e "${GREEN}Starting No-KVM VPS...${RESET}"
    ...
    echo
    echo -e "${LIGHT_GREEN}NO-KVM VPS CREATED SUCCESSFULLY${RESET}"
    echo
    ...
    echo
    echo -e "${YELLOW}No-KVM performance can be slow.${RESET}"
}
